"""
Translations of the bot's interface text AND of the intro sentence for the
cross-links in the captions. These are STATIC texts, written once and for
all here — never generated by DeepSeek (which is only used for the video
title/description content, see translator.py).
"""
import config

# Sentence shown before the list of available languages, in the language of
# EACH channel. The "original" channel uses the French sentence (it has no
# language of its own).
CROSS_LINK_INTRO = {
...
# Note shown in the caption of each dubbed channel (never on "original",
# which IS the editable version), explaining that an editable/voice-free
# version exists, for anyone who would want to translate it into another
# language. Idiomatic translation per language rather than literal (e.g.
# "NB" becomes "Hinweis" in German, "Примечание" in Russian, etc. — never
# an em dash).
...
# Label of the native hyperlink pointing to the "original" channel.
...
# Name of each language (channel key), translated into EVERY possible
# interface language. Unlike config.DISPLAY_NAMES (always in native script,
# e.g. "Deutsch"), this is used to display "Allemand"/"German"/"Alemão"...
# depending on the recipient's interface language (used in the newsletter).
LANGUAGE_NAMES = {
    "fr": {
...
def language_name(lang_key: str, ui_language: str) -> str:
    """Name of lang_key, translated into ui_language (falls back to config.DISPLAY_NAMES)."""
